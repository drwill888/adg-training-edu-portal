...
export async function addToIcegramList({ email, firstName, lastName, interest, config } = {}) {
  ...
  try {
    ...
    for (const attempt of attempts) {
      ...
      // Page attempt: Icegram renders es_subscription_message with class "success"
      //   on success, "error" on duplicate/rate-limit, absent on nonce failure.
      //   We match the exact CSS class to avoid false-positives from "success"
      //   appearing elsewhere in the page (e.g., CSS selectors, class names).
      //   "Email Address already exists" also counts as ok — they're already on the list.
      const looksLikeSuccess = attempt.isPage
        ? res.ok && (
            /class="es_subscription_message success"/.test(trimmed) ||
            /Email Address already exists/i.test(trimmed)
          )
        : res.ok &&
          trimmed !== "0" &&
          trimmed !== "-1" &&
      ...
      // For page attempts log the subscription message content, not raw HTML
      const logSnippet = attempt.isPage
        ? (trimmed.match(/class="es_subscription_message[^"]*"[^>]*>([^<]*)<\/span>/)?.[1]?.trim() || "(no subscription message in response)")
