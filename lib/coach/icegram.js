...
export async function addToIcegramList({ email, firstName, lastName, interest, config } = {}) {
  ...
  try {
    const { nonce } = await fetchNonce(formPageUrl);
    ...
    // Always derive the identifier from the target formId, not what's on the page.
    // WordPress nonces are site-wide so the page doesn't need to host the form.
    body.append("esfpx_es_form_identifier", `f${formId}-n1`);
    ...
    for (const attempt of attempts) {
      ...
      const looksLikeSuccess =
        ...
        trimmed !== "0" &&
        trimmed !== "-1" &&
        !trimmed.startsWith("<!") &&          // reject HTML page responses (false positive)
        !trimmed.startsWith("<html") &&        // reject HTML page responses (false positive)
